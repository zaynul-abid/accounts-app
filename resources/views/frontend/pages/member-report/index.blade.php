...
<div class="max-w-7xl mx-auto px-4 py-6 lg:py-8">
    <div class="mb-6 rounded-2xl bg-white shadow-sm border border-slate-200 p-5 lg:p-6 flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl lg:text-3xl font-bold text-slate-900">Member Reports</h1>
            <p class="text-sm text-slate-500 mt-1">View and manage all member financial transactions</p>
        </div>
        <div class="flex gap-2">
            ...
            <a href="{{ route('member-reports.yearly-payment.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 text-white px-4 py-2 text-sm font-medium hover:bg-blue-700">
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-slate-700">Member Name</th>
                    ...
                    @forelse($reports as $report)
                        <tr class="border-b border-slate-200 hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 font-semibold text-slate-900">
                                <a href="{{ route('member-reports.show-member', $report->member_id) }}" class="text-emerald-600 hover:text-emerald-700">
                                    {{ $report->name }}
                                </a>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-8 text-center text-slate-500">
                                ...
                                <p>No member reports found. <a href="{{ route('member-reports.create') }}" class="text-emerald-600 hover:text-emerald-700">Create one</a></p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
