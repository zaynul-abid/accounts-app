    ...
    <div class="flex-1 flex items-center justify-center p-6 md:p-12 bg-white">
        <div class="w-full max-w-md">
            ...
            <form id="loginForm" method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div>
                    <label for="login" class="block text-sm font-semibold text-gray-700 mb-2">Username or Email</label>
                    <input type="text" id="login" name="login"
                           class="w-full px-4 py-3.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-800 focus:border-transparent outline-none transition-all @error('login') border-red-500 @enderror"
                           placeholder="username or name@email.com" required autofocus value="{{ old('login') }}">
                    @error('login')
